...
@property (nonatomic, strong) DLSegmentBarConfig *config;

@end

@implementation DLSegmentBar



#pragma mark - 接口
+ (instancetype)segmentBarWithFrame: (CGRect)frame {
    DLSegmentBar *segmentBar = [[DLSegmentBar alloc] initWithFrame:frame];
...
- (void)btnClick: (UIButton *)btn {
    ...
    [UIView animateWithDuration:0.1 animations:^{
        self.indicatorView.dl_width = btn.dl_width + self.config.indicatorExtraW * 2;
        self.indicatorView.dl_centerX = btn.dl_centerX;
    }];
    
    
    // 1. 县滚动到btn的位置

    CGFloat scrollX = btn.dl_centerX - self.contentView.dl_width * 0.5;
...
- (void)layoutSubviews {
    ...
    self.indicatorView.dl_width = btn.dl_width + self.config.indicatorExtraW * 2;
    self.indicatorView.dl_centerX = btn.dl_centerX;
    self.indicatorView.dl_height = self.config.indicatorHeight;
...
- (DLSegmentBarConfig *)config {
    if (!_config) {
        _config = [DLSegmentBarConfig defaultConfig];
    }
    return _config;
}
